...
type KubeletFlags struct {
	KubeConfig                     string // 与api server 通信的地址
...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.BoolVar(&c.FailSwapOn, "fail-swap-on", c.FailSwapOn, "")
	...
	fs.DurationVar(&c.SyncFrequency.Duration, "sync-frequency", c.SyncFrequency.Duration, "周期性全量同步容器、配置的间隔")
	...
	fs.StringVar(&c.PodCIDR, "pod-cidr", c.PodCIDR, "在独立模式下,需要手动设置该 CIDR,用于分配 Pod 的 IP 地址.而在集群模式下,该 CIDR 是从主节点获取的,不需要手动设置.对于 IPv6,最大分配的 IP 数量是65536.")
	fs.Int64Var(&c.PodPidsLimit, "pod-max-pids", c.PodPidsLimit, "设置每个pod的最大进程数.如果是-1,kubelet默认为节点可分配的pid容量.")
	fs.StringVar(&c.ResolverConfig, "resolv-conf", c.ResolverConfig, "dns解析配置文件的路径")
	...
	fs.Var(cliflag.NewMapStringStringNoSplit(&c.TopologyManagerPolicyOptions), "topology-manager-policy-options", "关于拓扑管理器策略选项的键值对集合,用于调整其行为")
