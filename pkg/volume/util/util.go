...
const (
	...
	// KeepTerminatedPodVolumesAnnotation 声明了一个字符串常量,用于决定当Pod终止时是否卸载Pod卷的注释键.
	KeepTerminatedPodVolumesAnnotation string = "volumes.kubernetes.io/keep-terminated-pod-volumes"
	// MountsInGlobalPDPath 声明了一个字符串常量,用于在全局PD路径中创建用于卷挂载的目录.
	MountsInGlobalPDPath = "mounts"
	// VolumeGidAnnotationKey 声明了一个字符串常量,用于指定PersistentVolume对象的附加GID.
	VolumeGidAnnotationKey = "pv.beta.kubernetes.io/gid"
	// VolumeDynamicallyCreatedByKey 声明了一个字符串常量,用于指定动态创建的PersistentVolume对象的键.
	VolumeDynamicallyCreatedByKey = "kubernetes.io/createdby"
	// 声明了一个字符串常量,用于指定kubernetes插件挂载路径的前缀.
...
// IsDeviceMountableVolume 是否是可设备挂载的卷
...
func GetPodVolumeNames(pod *v1.Pod) (mounts sets.String, devices sets.String, seLinuxContainerContexts map[string][]*v1.SELinuxOptions) {
	...
	podutil.VisitContainers(&pod.Spec, podutil.AllFeatureEnabledContainers(), func(container *v1.Container, containerType podutil.ContainerType) bool {
		...
		if utilfeature.DefaultFeatureGate.Enabled(features.SELinuxMountReadWriteOncePod) {
			effectiveContainerSecurity := securitycontext.DetermineEffectiveSecurityContext(pod, container) // pod、container  sc的组合体
...
func GetUniqueVolumeNameFromSpecWithPod(podName types.UniquePodName, volumePlugin volume.VolumePlugin, volumeSpec *volume.Spec) v1.UniqueVolumeName {
	return v1.UniqueVolumeName(fmt.Sprintf("%s/%v-%s", volumePlugin.GetPluginName(), podName, volumeSpec.Name()))
}
...
func UnmapBlockVolume(blkUtil volumepathhandler.BlockVolumePathHandler, globalUnmapPath, podDeviceUnmapPath, volumeMapName string, podUID utypes.UID) error {
	...
	if err != nil {
		return fmt.Errorf("blkUtil.DetachFileDevice failed. globalUnmapPath:%s, podUID: %s: %v", globalUnmapPath, string(podUID), err)
	}
...
// MapBlockVolume 将指定卷和 pod 的块设备路径映射为常见的方式
func MapBlockVolume(
	...
	//将 devicePath 映射到全局节点路径作为绑定挂载.这意味着将设备路径挂载到全局节点路径,以便在整个节点上访问该设备.
	//这样做的一个常见原因是,如果多个 Pod 都需要访问同一个设备,那么将设备挂载到全局节点路径上可以避免在每个 Pod 中都进行设备挂载,从而提高效率.
	...
	// 将 devicePath 映射到 Pod 卷路径.这意味着将设备挂载到 Pod 的卷路径上,以便该 Pod 可以访问该设备.
	...
	// 获取文件描述符锁,以保持块设备处于打开状态.否则,有一种情况是块设备被静默地删除并附加了另一个具有相同名称的设备.容器运行时无法处理此问题.为了避免意外情况,需要对块设备进行文件描述符锁定.
...
func IsLocalEphemeralVolume(volume v1.Volume) bool {
	return volume.GitRepo != nil || (volume.EmptyDir != nil && volume.EmptyDir.Medium == v1.StorageMediumDefault) || volume.ConfigMap != nil
}
...
// IsMultiAttachAllowed 该函数检查是否绝对不允许/可能将此卷附加到多个节点.
// 在其当前形式下,此函数只能可靠地说明哪些卷是绝对禁止的.如果返回false,则不能保证多个附加实际上受到卷类型的支持,我们必须依赖于附加程序在这些情况下快速失败.
