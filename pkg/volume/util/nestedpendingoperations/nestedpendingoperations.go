...
// NestedPendingOperations 嵌套待处理操作
type NestedPendingOperations interface {

	// Run 对 同一 volumeName  podName nodeName的操作要串行处理
...
func (grm *nestedPendingOperations) IsOperationSafeToRetry(volumeName v1.UniqueVolumeName, podName volumetypes.UniquePodName, nodeName types.NodeName, operationName string) bool {
...
func (grm *nestedPendingOperations) IsOperationPending(volumeName v1.UniqueVolumeName, podName volumetypes.UniquePodName, nodeName types.NodeName) bool {
...
func (grm *nestedPendingOperations) Run(volumeName v1.UniqueVolumeName, podName volumetypes.UniquePodName, nodeName types.NodeName, generatedOperations volumetypes.GeneratedOperations) error {
	grm.lock.Lock()
	defer grm.lock.Unlock()
...
func (grm *nestedPendingOperations) operationComplete(key operationKey, err *error) {
	...
	if getOpErr != nil {
		...
		klog.Errorf("Operation %+v completed. error: %v. exponentialBackOffOnError is enabled, but failed to get operation to update.", key, *err)
		return
	}
	...
	// Log error
	klog.Errorf("%v", grm.operations[existingOpIndex].expBackoff.GenerateNoRetriesPermittedMsg(fmt.Sprintf("%+v", key)))
}
...
func (grm *nestedPendingOperations) isOperationExists(key operationKey) (bool, int) {
	...
	for previousOpIndex, previousOp := range grm.operations {
		...
		podNameMatch := previousOp.key.podName == EmptyUniquePodName || key.podName == EmptyUniquePodName || previousOp.key.podName == key.podName
		podNameExactMatch := previousOp.key.podName == key.podName
		nodeNameMatch := previousOp.key.nodeName == EmptyNodeName || key.nodeName == EmptyNodeName || previousOp.key.nodeName == key.nodeName
