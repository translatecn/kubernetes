...
type operationGenerator struct {
	...
	blkUtil         volumepathhandler.BlockVolumePathHandler // 一些与块卷相关的路径操作
...
func NewOperationGenerator(kubeClient clientset.Interface, volumePluginMgr *volume.VolumePluginMgr, recorder record.EventRecorder, blkUtil volumepathhandler.BlockVolumePathHandler) OperationGenerator {
...
type OperationGenerator interface {
	...
	GenerateBulkVolumeVerifyFunc(map[types.NodeName][]*volume.Spec, string, map[*volume.Spec]v1.UniqueVolumeName, ActualStateOfWorldAttacherUpdater) (volumetypes.GeneratedOperations, error)
...
func (og *operationGenerator) GenerateVolumesAreAttachedFunc(attachedVolumes []AttachedVolume, nodeName types.NodeName, actualStateOfWorld ActualStateOfWorldAttacherUpdater) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) GenerateBulkVolumeVerifyFunc(pluginNodeVolumes map[types.NodeName][]*volume.Spec, pluginName string, volumeSpecMap map[*volume.Spec]v1.UniqueVolumeName, actualStateOfWorld ActualStateOfWorldAttacherUpdater) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) GenerateDetachVolumeFunc(volumeToDetach AttachedVolume, verifySafeToDetach bool, actualStateOfWorld ActualStateOfWorldAttacherUpdater) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) GenerateUnmountDeviceFunc(deviceToDetach AttachedVolume, actualStateOfWorld ActualStateOfWorldMounterUpdater, hostutil hostutil.HostUtils) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) GenerateUnmapDeviceFunc(deviceToDetach AttachedVolume, actualStateOfWorld ActualStateOfWorldMounterUpdater, hostutil hostutil.HostUtils) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) verifyVolumeIsSafeToDetach(volumeToDetach AttachedVolume) error {
...
func (og *operationGenerator) GenerateExpandVolumeFunc(pvc *v1.PersistentVolumeClaim, pv *v1.PersistentVolume) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) GenerateExpandAndRecoverVolumeFunc(pvc *v1.PersistentVolumeClaim, pv *v1.PersistentVolume, resizerName string) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) expandVolumeDuringMount(volumeToMount VolumeToMount, actualStateOfWorld ActualStateOfWorldMounterUpdater, rsOpts volume.NodeResizeOptions) (bool, error) {
	...
	if supportsExpansion {
		...
		if pvcStatusCap.Cmp(pvSpecCap) < 0 { //  pvcStatusCap <  pvSpecCap
...
// legacyCallNodeExpandOnPlugin 是在插件上调用节点扩展的旧版本,不支持从卷扩展失败中恢复.
...
// checkNodeAffinity 检查pv 是否可以被这个节点访问
func checkNodeAffinity(og *operationGenerator, volumeToMount VolumeToMount) error {
	...
	if pv != nil {
		...
		err = storagehelpers.CheckNodeAffinity(pv, nodeLabels) // 检查pv 是否可以被这个节点访问
		if err != nil {
			return err
		}
	}
...
// GenerateUnmountVolumeFunc 执行卷插件的卸载操作
func (og *operationGenerator) GenerateUnmountVolumeFunc(volumeToUnmount MountedVolume, actualStateOfWorld ActualStateOfWorldMounterUpdater, podsDir string) (volumetypes.GeneratedOperations, error) {
	...
	volumeUnmounter, newUnmounterErr := volumePlugin.NewUnmounter(volumeToUnmount.InnerVolumeSpecName, volumeToUnmount.PodUID)
	...
	unmountVolumeFunc := func() volumetypes.OperationContext {
		...
		// 移除的是/var/lib/kubelet/pods/<uid>/volume-subpaths/<volume>/*
		...
		// Update actual state of world
		markVolMountedErr := actualStateOfWorld.MarkVolumeAsUnmounted(volumeToUnmount.PodName, volumeToUnmount.VolumeName)
...
// 从pod设备映射路径目录和全局映射路径目录中删除符号链接.
// 完成上述清理后,删除pod设备映射路径目录.
// 如果所有步骤都完成,那么该卷会被标记为未挂载.
func (og *operationGenerator) GenerateUnmapVolumeFunc(volumeToUnmount MountedVolume, actualStateOfWorld ActualStateOfWorldMounterUpdater) (volumetypes.GeneratedOperations, error) {
	...
	blockVolumeUnmapper, newUnmapperErr := blockVolumePlugin.NewBlockVolumeUnmapper(volumeToUnmount.InnerVolumeSpecName, volumeToUnmount.PodUID)
	if newUnmapperErr != nil {
...
func (og *operationGenerator) GenerateVerifyControllerAttachedVolumeFunc( // ✅
	volumeToMount VolumeToMount, nodeName types.NodeName, actualStateOfWorld ActualStateOfWorldAttacherUpdater,
	...
	//对于可附加的卷类型,通过从节点列表读取来检查卷是否已附加.
	//这将避免指数退避和不必要的 goroutine 创建.
	//我们仍然通过直接从 API 服务器读取来验证已附加卷的状态.这是必要的,以确保由于通知器中的缓存状态而引起的任何竞争条件.
	...
	verifyControllerAttachedVolumeFunc := func() volumetypes.OperationContext {
		...
		// 如果该卷插件 没有实现 attacher 接口,则假定该卷已附加,并相应地更新实际世界的状态.
		if !volumeToMount.PluginIsAttachable {
			addVolumeNodeErr := actualStateOfWorld.MarkVolumeAsAttached(volumeToMount.VolumeName, volumeToMount.VolumeSpec, nodeName, "" /* devicePath */)
		...
		if !volumeToMount.ReportedInUse {
			// 如果指定的卷还没有被添加到节点的卷状态的VolumesInUse列表中,则不继续执行,返回错误.调用者会记录并重试.
			// 节点状态由kubelet定期更新,因此可能需要多达10秒才能清除此问题.有关此问题的更多信息,请参见问题＃28141以启用按需状态更新.
		...
		if node == nil {
			// On failure, return error. Caller will log and retry.
			eventErr, detailedErr := volumeToMount.GenerateError("VerifyControllerAttachedVolume failed", fmt.Errorf("node object retrieved from API server is nil"))
			return volumetypes.NewOperationContext(eventErr, detailedErr, migrated)
		}

		for _, attachedVolume := range node.Status.VolumesAttached {
			if attachedVolume.Name == volumeToMount.VolumeName {
				addVolumeNodeErr := actualStateOfWorld.MarkVolumeAsAttached(v1.UniqueVolumeName(""), volumeToMount.VolumeSpec, nodeName, attachedVolume.DevicePath)
...
func (og *operationGenerator) markDeviceErrorState(volumeToMount VolumeToMount, devicePath, deviceMountPath string, mountError error, actualStateOfWorld ActualStateOfWorldMounterUpdater) {
	if volumetypes.IsOperationFinishedError(mountError) && actualStateOfWorld.GetDeviceMountState(volumeToMount.VolumeName) == DeviceMountUncertain {
	...
	if volumetypes.IsUncertainProgressError(mountError) && actualStateOfWorld.GetDeviceMountState(volumeToMount.VolumeName) == DeviceNotMounted {
...
func (og *operationGenerator) checkIfSupportsNodeExpansion(volumeToMount VolumeToMount) (bool, volume.NodeExpandableVolumePlugin) {
	if volumeToMount.VolumeSpec != nil && volumeToMount.VolumeSpec.InlineVolumeSpecForCSIMigration {
		klog.V(4).Infof("这个卷 %s  是一个已迁移的内联卷,无法调整大小.", volumeToMount.VolumeName)
		return false, nil
	}

	// Get expander, if possible
	expandableVolumePlugin, _ := og.volumePluginMgr.FindNodeExpandablePluginBySpec(volumeToMount.VolumeSpec)
	if expandableVolumePlugin != nil && expandableVolumePlugin.RequiresFSResize() && volumeToMount.VolumeSpec.PersistentVolume != nil {
...
func (og *operationGenerator) GenerateMountVolumeFunc( // ✅
	waitForAttachTimeout time.Duration, volumeToMount VolumeToMount, actualStateOfWorld ActualStateOfWorldMounterUpdater, isRemount bool) volumetypes.GeneratedOperations {
	...
	mountVolumeFunc := func() volumetypes.OperationContext {
		...
		affinityErr := checkNodeAffinity(og, volumeToMount) // ✅ pv 节点亲和性
		if affinityErr != nil {
		...
		mountCheckError := checkMountOptionSupport(og, volumeToMount, volumePlugin) // ✅ 检查 volumePlugin 是否支持配置项设置
		...
		// 如果只有ReadWriteOncePod访问模式存在,则强制执行它.这也在调度期间强制执行.
		if utilfeature.DefaultFeatureGate.Enabled(features.ReadWriteOncePod) && // 是否开启了一个卷只能让一个pod访问
			actualStateOfWorld.IsVolumeMountedElsewhere(volumeToMount.VolumeName, volumeToMount.PodName) && // volume 是否被其他 pod 挂载
			len(volumeToMount.VolumeSpec.PersistentVolume.Spec.AccessModes) == 1 && // 只设置了一种访问模式
			v1helper.ContainsAccessMode(volumeToMount.VolumeSpec.PersistentVolume.Spec.AccessModes, v1.ReadWriteOncePod) {

			err = goerrors.New("该卷使用了ReadWriteOncePod访问模式,并且已被另一个Pod使用.由于该访问模式只允许一个Pod使用该卷,因此该卷不能被其他Pod使用.")
			eventErr, detailedErr := volumeToMount.GenerateError("MountVolume.SetUp failed", err)
			return volumetypes.NewOperationContext(eventErr, detailedErr, migrated)
		}

		// Get attacher, if possible
		var _ = new(volume.VolumePluginMgr).FindAttachablePluginBySpec
		...
		resizeOptions := volume.NodeResizeOptions{DevicePath: devicePath}
		...
		if volumeDeviceMounter != nil && actualStateOfWorld.GetDeviceMountState(volumeToMount.VolumeName) != DeviceGloballyMounted {
			...
			markDeviceMountedErr := actualStateOfWorld.MarkDeviceAsMounted(volumeToMount.VolumeName, devicePath, deviceMountPath, volumeToMount.SELinuxLabel)
		...
		// 如果不使用用户命名空间（userns）,则不需要为 hostUID/hostGID 进行映射.
		// 因此,可以将容器用户直接分配给主机的 UID/GID.这段代码的作用是判断是否使用用户命名空间,如果没有使用,则直接将容器用户分配给主机的 UID/GID.
		...
		_, resizeError = og.expandVolumeDuringMount(volumeToMount, actualStateOfWorld, resizeOptions) // ✅ 指定在挂载卷时自动扩展卷的容量
		if resizeError != nil {
		...
		totalTimeTaken := time.Since(volumeToMount.MountRequestTime).Seconds()
...
func (og *operationGenerator) GenerateMapVolumeFunc(waitForAttachTimeout time.Duration, volumeToMount VolumeToMount, actualStateOfWorld ActualStateOfWorldMounterUpdater) (volumetypes.GeneratedOperations, error) {
	...
	affinityErr := checkNodeAffinity(og, volumeToMount) // ✅ 检查pv 是否可以被这个节点访问
	if affinityErr != nil {
	...
	mapVolumeFunc := func() (operationContext volumetypes.OperationContext) {
		...
		if ok && actualStateOfWorld.GetDeviceMountState(volumeToMount.VolumeName) != DeviceGloballyMounted {
			var mapErr error
		...
		markDeviceMappedErr := actualStateOfWorld.MarkDeviceAsMounted(volumeToMount.VolumeName, markedDevicePath, globalMapPath, "")
		...
		// 当kubelet被容器化时,devicePath可能是一个符号链接
		// kubelet,所以在主机上评估它,并期望它链接到/dev中的设备,
		// 容器化的kubelet可以使用.如果它仍然不存在,
		// AttachFileDevice将失败.如果kubelet没有容器化,无论如何都要对其进行eval.
		kvh, ok := og.GetVolumePluginMgr().Host.(volume.KubeletVolumeHost)
		if !ok {
		...
		if markedDevicePath != devicePath {
			markDeviceMappedErr := actualStateOfWorld.MarkDeviceAsMounted(volumeToMount.VolumeName, devicePath, globalMapPath, "")
			if markDeviceMappedErr != nil {
				// On failure, return error. Caller will log and retry.
				eventErr, detailedErr := volumeToMount.GenerateError("MapVolume.MarkDeviceAsMounted failed", markDeviceMappedErr)
				return volumetypes.NewOperationContext(eventErr, detailedErr, migrated)
...
func (og *operationGenerator) GenerateExpandInUseVolumeFunc(volumeToMount VolumeToMount, actualStateOfWorld ActualStateOfWorldMounterUpdater, currentSize resource.Quantity) (volumetypes.GeneratedOperations, error) {
...
func (og *operationGenerator) nodeExpandVolume(volumeToMount VolumeToMount, actualStateOfWorld ActualStateOfWorldMounterUpdater, rsOpts volume.NodeResizeOptions) (bool, error) {
...
func (og *operationGenerator) GenerateAttachVolumeFunc( // ✅
	volumeToAttach VolumeToAttach, actualStateOfWorld ActualStateOfWorldAttacherUpdater) volumetypes.GeneratedOperations {
