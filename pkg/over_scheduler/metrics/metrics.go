...
import (
	...
	volumebindingmetrics "k8s.io/kubernetes/pkg/over_scheduler/framework/plugins/volumebinding/metrics"
)

const (
	// SchedulerSubsystem - subsystem name used by over_scheduler
	SchedulerSubsystem = "over_scheduler"
...
var (
	scheduleAttempts = metrics.NewCounterVec(
		&metrics.CounterOpts{
			...
			Help:           "Number of attempts to schedule pods, by the result. 'unschedulable' means a pod could not be scheduled, while 'error' means an internal over_scheduler problem.",
	...
	pendingPods = metrics.NewGaugeVec(
		&metrics.GaugeOpts{
			Subsystem:      SchedulerSubsystem,
			Name:           "pending_pods",
			Help:           "Number of pending pods, by the queue type. 'active' means number of pods in activeQ; 'backoff' means number of pods in backoffQ; 'unschedulable' means number of pods in unschedulablePods that the over_scheduler attempted to schedule and failed; 'gated' is the number of unschedulable pods that the over_scheduler never attempted to schedule because they are gated.",
			StabilityLevel: metrics.STABLE,
		}, []string{"queue"})
	...
	CacheSize = metrics.NewGaugeVec(
		&metrics.GaugeOpts{
			...
			Help:           "Number of nodes, pods, and assumed (bound) pods in the over_scheduler cache.",
			StabilityLevel: metrics.ALPHA,
		}, []string{"type"})
