...
package over_scheduler

import (
	"context"
	"fmt"
	"k8s.io/kubernetes/pkg/over_scheduler/metrics"
	"math/rand"
	"strconv"
	"sync"
	"sync/atomic"
	...
	"k8s.io/kubernetes/pkg/over_scheduler/framework"
	"k8s.io/kubernetes/pkg/over_scheduler/framework/parallelize"
	frameworkruntime "k8s.io/kubernetes/pkg/over_scheduler/framework/runtime"
	internalqueue "k8s.io/kubernetes/pkg/over_scheduler/internal/queue"
	"k8s.io/kubernetes/pkg/over_scheduler/util"
...
func (sched *Scheduler) schedulePod(ctx context.Context, fwk framework.Framework, state *framework.CycleState, pod *v1.Pod) (result ScheduleResult, err error) {
	...
	trace.Step("Snapshotting over_scheduler cache and node infos done")
...
func prioritizeNodes(
	...
	if len(extenders) != 0 && nodes != nil {
		...
		wg.Wait()
		for i := range result {
			// MaxExtenderPriority may diverge from the max priority used in the over_scheduler and defined by MaxNodeScore,
			// therefore we need to scale the score returned by extenders to the score range used by the over_scheduler.
...
func (sched *Scheduler) frameworkForPod(pod *v1.Pod) (framework.Framework, error) {
	...
	if !ok {
		return nil, fmt.Errorf("profile not found for over_scheduler name %q", pod.Spec.SchedulerName)
	}
	return fwk, nil
}
...
func (sched *Scheduler) scheduleOne(ctx context.Context) {
	...
	if err != nil {
		...
		// which specify a over_scheduler name that matches one of the profiles.
...
func (sched *Scheduler) handleBindingCycleError(
	...
	fwk.RunReservePluginsUnreserve(ctx, state, assumedPod, scheduleResult.SuggestedHost)
	if forgetErr := sched.Cache.ForgetPod(assumedPod); forgetErr != nil {
		klog.ErrorS(forgetErr, "over_scheduler cache ForgetPod failed")
	} else {
		// "Forget"ing an assumed Pod in binding cycle should be treated as a PodDelete event,
		// as the assumed Pod had occupied a certain amount of resources in over_scheduler cache.
		//
...
func (sched *Scheduler) assume(assumed *v1.Pod, host string) error {
	...
	// in the background.
	// If the binding fails, over_scheduler will release resources allocated to assumed pod
	// immediately.
	// 乐观地假设绑定将成功，并在后台将其发送到API服务器。
	// 如果绑定失败，调度器将立即释放分配给假定Pod的资源。
...
func (sched *Scheduler) handleSchedulingFailure(ctx context.Context, fwk framework.Framework, podInfo *framework.QueuedPodInfo, err error, reason string, nominatingInfo *framework.NominatingInfo, start time.Time) {
	...
	if err == ErrNoNodesAvailable {
		...
		if errStatus, ok := err.(apierrors.APIStatus); ok && errStatus.Status().Details.Kind == "node" {
			...
			// the node and if the node is still not found, then remove it from the over_scheduler cache.
	...
	if e != nil {
		...
		// In the case of extender, the pod may have been bound successfully, but timed out returning its response to the over_scheduler.
	...
	// and the time the over_scheduler receives a Pod Update for the nominated pod.
...
// numFeasibleNodesToFind returns the number of feasible nodes that once found, the over_scheduler stops
// its search for more feasible nodes.  调整可发现的node 数
...
func (sched *Scheduler) findNodesThatPassFilters(
	...
	errCh := over_parallelize.NewErrorChannel()
...
func (sched *Scheduler) findNodesThatFitPod(ctx context.Context, fwk framework.Framework, state *framework.CycleState, pod *v1.Pod) ([]*v1.Node, framework.Diagnosis, error) {
	...
	// 由于抢占，“NominatedNodeName”可能会在之前的调度周期中设置。该节点可能是适合pod的唯一候选节点，因此我们在遍历所有节点之前先尝试它。
	if len(pod.Status.NominatedNodeName) > 0 {
		feasibleNodes, err := sched.evaluateNominatedNode(ctx, pod, fwk, state, diagnosis)
		if err != nil {
			klog.ErrorS(err, "Evaluation failed on nominated node", "pod", klog.KObj(pod), "node", pod.Status.NominatedNodeName)
		}
		// Nominated node passes all the filters, over_scheduler is good to assign this node to the pod.
		if len(feasibleNodes) != 0 {
...
func findNodesThatPassExtenders(extenders []framework.Extender, pod *v1.Pod, feasibleNodes []*v1.Node, statuses framework.NodeToStatusMap) ([]*v1.Node, error) {
	...
	for _, extender := range extenders {
		...
		// so that the over_scheduler framework can respect the UnschedulableAndUnresolvable status for
