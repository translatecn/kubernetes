...
const (
	...
	Name = over_names.DynamicResources
...
type stateData struct {
	...
	// Gets set on demand.
	//
	// Conceptually, this object belongs into the over_scheduler framework
...
type dynamicResources struct {
	enabled             bool // pod绑定节点后,需不需要通知 资源提供方 bang
...
func New(plArgs runtime.Object, fh framework.Handle, fts over_feature.Features) (framework.Plugin, error) {
...
func (pl *dynamicResources) Reserve(ctx context.Context, cs *framework.CycleState, pod *v1.Pod, nodeName string) *framework.Status {
	...
	if len(podScheduling.Spec.PotentialNodes) == 0 {
		...
		// node is suitable, otherwise the over_scheduler will pick
	...
	if numDelayedAllocationPending == 1 || numClaimsWithStatusInfo == numDelayedAllocationPending {
		podScheduling = podScheduling.DeepCopy()
		// TODO: can we increase the chance that the over_scheduler picks
