...
import (
	...
	"k8s.io/kubernetes/pkg/over_scheduler/framework/plugins/volumebinding/metrics"
)
...
// SchedulerVolumeBinder is used by the over_scheduler VolumeBinding plugin to
...
//
// This integrates into the existing over_scheduler workflow as follows:
//  1. The over_scheduler takes a Pod off the over_scheduler queue and processes it serially:
...
//     ii. Afterwards, the main over_scheduler caches the Pod->Node binding in the over_scheduler's pod cache,
//     This is handled in the over_scheduler and not here.
...
//     back through the over_scheduler.
//     ii. After BindPodVolumes() is complete, then the over_scheduler does the final Pod->Node binding.
//  2. Once all the assume operations are done in e), the over_scheduler processes the next Pod in the over_scheduler queue
...
type SchedulerVolumeBinder interface {
	...
	//
	// This function is called by the over_scheduler VolumeBinding plugin and can be called in parallel
	...
	// BindPodVolumes will:
	// 1. 通过调用 API 启动卷绑定，将持久卷（PV）与其匹配的持久卷声明（PVC）进行预绑定。
	// 2. 通过调用 API，在持久卷声明（PVC）上设置相关的注释，触发卷预配。
	// 3. 等待 PV 控制器完全将持久卷声明（PVC）与持久卷（PV）绑定。
	//
	// 此函数可以并行调用。
...
// NewVolumeBinder sets up all the caches needed for the over_scheduler to make volume binding decisions.
//
...
func (b *volumeBinder) tryTranslatePVToCSI(pv *v1.PersistentVolume, csiNode *storagev1.CSINode) (*v1.PersistentVolume, error) {
	//如果符合条件，`translate`函数将把in-tree PV转换为CSI。否则，它将返回未修改的in-tree PV。
...
// `checkBindings`函数的作用是遍历 Pod 中的所有持久卷声明（PVC），并进行以下检查：
// - PVC 是否完全绑定
// - 是否存在需要失败并进行重试的绑定条件
//
// 当 Pod 的所有 PVC 都完全绑定时，该函数返回 true；如果需要重新绑定（包括调度），则返回错误。
// 需要注意的是，该函数检查 API 对象而不是 PV/PVC 缓存。
// 这是因为 PV/PVC 缓存在主调度器循环中可以再次假定，我们必须检查 API 服务器中的最新状态，这些状态与 PV 控制器和提供程序共享。
func (b *volumeBinder) checkBindings(pod *v1.Pod, bindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim) (bool, error) {
	...
	for _, binding := range bindings {
		...
		// 因为我们在apisserver中更新了PV，所以跳过旧的API对象，等待从apisserver传播新的API对象。
		// 就是 watch 慢了
		if versioner.CompareResourceVersion(binding.pv, pv) > 0 { // binding.pv > pv
			return false, nil
		}
	...
	for _, claim := range claimsToProvision {
		...
		if selectedNode != pod.Spec.NodeName {
			...
			// annotation will be removed to signal back to the over_scheduler to
			// retry.
