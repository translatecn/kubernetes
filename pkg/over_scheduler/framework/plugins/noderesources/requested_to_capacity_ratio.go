...
import (
	...
	"k8s.io/kubernetes/pkg/over_scheduler/framework/plugins/helper"
)
...
func buildRequestedToCapacityRatioScorerFunction(scoringFunctionShape over_helper.FunctionShape, resourceToWeightMap resourceToWeightMap) func(resourceToValueMap, resourceToValueMap) int64 {
	rawScoringFunction := over_helper.BuildBrokenLinearFunction(scoringFunctionShape)
...
func requestedToCapacityRatioScorer(weightMap resourceToWeightMap, shape []config.UtilizationShapePoint) func(resourceToValueMap, resourceToValueMap) int64 {
	shapes := make([]over_helper.FunctionShapePoint, 0, len(shape))
	for _, point := range shape {
		shapes = append(shapes, over_helper.FunctionShapePoint{
			Utilization: int64(point.Utilization),
			// MaxCustomPriorityScore may diverge from the max score used in the over_scheduler and defined by MaxNodeScore,
			// therefore we need to scale the score returned by requested to capacity ratio to the score range
			// used by the over_scheduler.
