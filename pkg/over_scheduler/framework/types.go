...
import (
	...
	"k8s.io/klog/v2"
	schedutil "k8s.io/kubernetes/pkg/over_scheduler/util"
)
...
func (ce ClusterEvent) IsWildCard() bool { // 检查是否支持通配符语义
...
type QueuedPodInfo struct {
	*PodInfo
	...
	UnschedulablePlugins    sets.String // 如果一个Pod在调度周期中失败，记录失败的插件名称。
	Gated                   bool        // true -> 视为不需要进入 backoff 队列，直接进入 ActiveQ
}
