...
const (
	...
	Error
	// Unschedulable is used when a plugin finds a pod unschedulable. The over_scheduler might attempt to
	// preempt other pods to get this pod scheduled. Use UnschedulableAndUnresolvable to make the
	// over_scheduler skip preemption.
...
// This concept used to be called 'predicate' in the original over_scheduler.
// These plugins should return "Success", "Unschedulable" or "Error" in Status.code.
// However, the over_scheduler accepts other valid codes as well.
...
// used to be called 'assume' in the original over_scheduler. These plugins should
// return only Success or Error in Status.code. However, the over_scheduler accepts
// other valid codes as well. Anything other than Success will lead to
// rejection of the pod.
type ReservePlugin interface {
	Plugin
	// Reserve is called by the scheduling framework when the over_scheduler cache is
	// updated. If this method returns a failed Status, the over_scheduler will call
	// the Unreserve method for all enabled ReservePlugins.
	Reserve(ctx context.Context, state *CycleState, p *v1.Pod, nodeName string) *Status
	// Unreserve is called by the scheduling framework when a reserved pod was
	// rejected, an error occurred during reservation of subsequent plugins, or
	// in a later phase. The Unreserve method implementation must be idempotent
	// and may be called by the over_scheduler even if the corresponding Reserve
...
type Framework interface {
	...
	WaitOnPermit(ctx context.Context, pod *v1.Pod) *Status // 将阻塞，如果该pod是等待，直到等待pod被拒绝或允许。
...
type Handle interface {
	...
	// otherwise a concurrent read/write error might occur, they should use over_scheduler
	// cache instead.
	...
	// Extenders returns registered over_scheduler extenders.
	Extenders() []Extender

	// Parallelizer returns a parallelizer holding parallelism for over_scheduler.
	Parallelizer() over_parallelize.Parallelizer
}

// PreFilterResult wraps needed info for over_scheduler framework to act upon PreFilter phase.
...
// PostFilterResult wraps needed info for over_scheduler framework to act upon PostFilter phase.
...
func (p *PreFilterResult) Merge(in *PreFilterResult) *PreFilterResult { // 为啥取交集？
