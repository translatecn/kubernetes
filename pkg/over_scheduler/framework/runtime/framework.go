...
// frameworkImpl is the component responsible for initializing and running over_scheduler
// plugins.
...
// SnapshotSharedLister returns the over_scheduler's SharedLister of the latest NodeInfo
...
func NewFramework(r Registry, profile *config.KubeSchedulerProfile, stopCh <-chan struct{}, opts ...Option) (framework.Framework, error) {
	...
	if len(f.queueSortPlugins) != 1 {
		return nil, fmt.Errorf("only one queue sort plugin required for profile with over_scheduler name %q, but got %d", profile.SchedulerName, len(f.queueSortPlugins))
	}
	if len(f.bindPlugins) == 0 {
		return nil, fmt.Errorf("at least one bind plugin is needed for profile with over_scheduler name %q", profile.SchedulerName)
	}
