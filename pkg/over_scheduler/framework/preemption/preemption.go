...
// `Preempt`函数返回一个携带建议的`nominatedNodeName`和`Status`的`PostFilterResult`。返回的 `<PostFilterResult, Status>` 在不同的情境下有不同的语义：
//
// - `<nil, Error>`：表示这是一个短暂/罕见的错误，可能会在将来的周期中自愈。
//
// - `<nil, Unschedulable>`：这个状态通常是预期的，如抢占者正在等待受害者完全终止。
//
// 在上述两种情况下，返回一个空的`PostFilterResult`，以保持Pod的`nominatedNodeName`不变。
//
// - `<non-nil PostFilterResult, Unschedulable>`：表示即使通过抢占，Pod也无法调度。在这种情况下，返回一个非空的`PostFilterResult`，并且`result.NominatingMode`指示如何处理`nominatedNodeName`。
//
...
// node. In that case, over_scheduler will find a different host for the preemptor in subsequent scheduling cycles.
...
func (ev *Evaluator) prepareCandidate(ctx context.Context, c Candidate, pod *v1.Pod, pluginName string) *framework.Status {
	...
	// lets over_scheduler find another place for them.
