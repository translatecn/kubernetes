...
type cacheImpl struct {
	...
	mu          sync.RWMutex
	assumedPods sets.String // 一组假的pod 不会进入调度循环
...
// Dump produces a dump of the current over_scheduler cache. This is used for
...
func (cache *cacheImpl) UpdatePod(oldPod, newPod *v1.Pod) error {
	...
	if ok && !cache.assumedPods.Has(key) {
		if currState.pod.Spec.NodeName != newPod.Spec.NodeName {
			...
			klog.ErrorS(nil, "over_scheduler cache is corrupted and can badly affect scheduling decisions")
			klog.FlushAndExit(klog.ExitFlushTimeout, 1)
		}
		return cache.updatePod(oldPod, newPod)
	}
	return fmt.Errorf("pod %v(%v) is not added to over_scheduler cache, so cannot be updated", key, klog.KObj(oldPod))
}
...
func (cache *cacheImpl) RemovePod(pod *v1.Pod) error {
	...
	currState, ok := cache.podStates[key]
	if !ok {
		return fmt.Errorf("pod %v(%v) is not found in over_scheduler cache, so cannot be removed from it", key, klog.KObj(pod))
	}
	if currState.pod.Spec.NodeName != pod.Spec.NodeName {
		...
		if pod.Spec.NodeName != "" {
			// An empty NodeName is possible when the over_scheduler misses a Delete
...
func (cache *cacheImpl) GetPod(pod *v1.Pod) (*v1.Pod, error) {
	...
	if !ok {
		return nil, fmt.Errorf("pod %v(%v) does not exist in over_scheduler cache", key, klog.KObj(pod))
	}
...
// over_scheduler cache. This function assumes the lock to over_scheduler cache has been acquired.
