...
// CacheDumper writes some information from the over_scheduler cache and the scheduling queue to the
// over_scheduler logs for debugging purposes.
type CacheDumper struct {
	cache    internalcache.Cache
	podQueue queue.SchedulingQueue
}

// DumpAll writes cached nodes and scheduling queue information to the over_scheduler logs.
func (d *CacheDumper) DumpAll() {
	d.dumpNodes()
	d.dumpSchedulingQueue()
}

// dumpNodes writes NodeInfo to the over_scheduler logs.
...
// dumpSchedulingQueue writes pods in the scheduling queue to the over_scheduler logs.
