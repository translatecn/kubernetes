...
import (
	...
	"k8s.io/kubernetes/pkg/over_scheduler/internal/heap"
...
const (
	...
	// over_scheduler, update the ComponentConfig value in defaults.go
...
// This function is used for debugging purposes in the over_scheduler cache dumper and comparer.
...
// may be different than what over_scheduler has here. We should be able to find pods
...
type PriorityQueue struct {
	...
	lock                              sync.RWMutex
	cond                              sync.Cond
	activeQ                           *heap.Heap         // activeQ是一个堆结构，调度器会主动查看它来查找pod 计划。堆头是最高优先级的pod。
	podBackoffQ                       *heap.Heap         // 是按回退到期排序的堆。在调度器查看activeQ之前，已经完成回退的pod会从这个堆中弹出
	unschedulablePods                 *UnschedulablePods // 保存已被尝试并确定为不可调度的pod。
	schedulingCycle                   int64              // 表示调度周期的序号，并在弹出pod时递增。
	moveRequestCycle                  int64              // 当我们收到移动请求时，moveRequestCycle缓存调度周期的序列号。如果我们在收到移动请求时试图调度它们，那么在此调度周期内和之前的不可调度的pod将被放回activeQueue。
	clusterEventMap                   map[framework.ClusterEvent]sets.String
	preEnqueuePluginMap               map[string][]framework.PreEnqueuePlugin
	closed                            bool
...
func (p *PriorityQueue) movePodsToActiveOrBackoffQueue(podInfoList []*framework.QueuedPodInfo, event framework.ClusterEvent) {
	...
	for _, pInfo := range podInfoList {
		...
		// 由当前pod被拒绝的状态 以及重试的条件    以及 当前允许重新调度的条件  选出 可以调度的pod
		if len(pInfo.UnschedulablePlugins) != 0 && !p.podMatchesEvent(pInfo, event) {
			continue
		}
		pod := pInfo.Pod
		if p.isPodBackingoff(pInfo) { // 是否仍然需要等会 回退时间超时
			if err := p.podBackoffQ.Add(pInfo); err != nil {
