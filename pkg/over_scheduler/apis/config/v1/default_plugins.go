...
func applyFeatureGates(config *v1.Plugins) {
	if utilfeature.DefaultFeatureGate.Enabled(features.PodSchedulingReadiness) {
		config.MultiPoint.Enabled = append(config.MultiPoint.Enabled, v1.Plugin{Name: over_names.SchedulingGates})
	}
	if utilfeature.DefaultFeatureGate.Enabled(features.DynamicResourceAllocation) {
		...
		for i := range config.MultiPoint.Enabled {
			if config.MultiPoint.Enabled[i].Name == over_names.DefaultPreemption {
				extended := make([]v1.Plugin, 0, len(config.MultiPoint.Enabled)+1)
				extended = append(extended, config.MultiPoint.Enabled[:i]...)
				extended = append(extended, v1.Plugin{Name: over_names.DynamicResources})
				extended = append(extended, config.MultiPoint.Enabled[i:]...)
	...
	plugins := &v1.Plugins{
		MultiPoint: v1.PluginSet{
			Enabled: []v1.Plugin{
				...
				{Name: over_names.TaintToleration, Weight: pointer.Int32(3)},
				{Name: over_names.NodeAffinity, Weight: pointer.Int32(2)},
				...
				{Name: over_names.InterPodAffinity, Weight: pointer.Int32(2)},
