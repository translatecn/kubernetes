...
import (
	...
	"k8s.io/kubernetes/pkg/over_scheduler/internal/queue"
)
...
func (sched *Scheduler) deletePodFromSchedulingQueue(obj interface{}) {
	...
	// removing it from the over_scheduler cache. In this case, signal a AssignedPodDelete
...
// responsibleForPod returns true if the pod has asked to be scheduled by the given over_scheduler.
...
func addAllEventHandlers(
	...
	for gvk, at := range gvkMap {
		...
		case framework.PersistentVolume:
			...
			// parties, then over_scheduler will add pod back to unschedulable queue. We
...
// 它用于kubelet(pkg/kubelet/lifecycle/over-predicate.go)和 over_scheduler.如果' includeAllFailures '设置为false,则返回第一个失败;否则返回所有失败.
func AdmissionCheck(pod *v1.Pod, nodeInfo *framework.NodeInfo, includeAllFailures bool) []AdmissionResult {
	...
	if !over_nodeports.Fits(pod, nodeInfo) {
		admissionResults = append(admissionResults, AdmissionResult{Name: over_nodeports.Name, Reason: over_nodeports.ErrReason})
		if !includeAllFailures {
			return admissionResults
		}
	}
