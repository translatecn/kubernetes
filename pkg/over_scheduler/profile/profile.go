...
// RecorderFactory builds an EventRecorder for a given over_scheduler name.
...
func (v *cfgValidator) validate(cfg config.KubeSchedulerProfile, f framework.Framework) error {
	if len(f.ProfileName()) == 0 {
		return errors.New("over_scheduler name is needed")
	}
	if cfg.Plugins == nil {
		return fmt.Errorf("plugins required for profile with over_scheduler name %q", f.ProfileName())
	}
	if v.m[f.ProfileName()] != nil {
		return fmt.Errorf("duplicate profile with over_scheduler name %q", f.ProfileName())
	}
...
func NewMap(cfgs []config.KubeSchedulerProfile, r frameworkruntime.Registry, recorderFact RecorderFactory,
	...
	for _, cfg := range cfgs {
		...
		if err != nil {
			return nil, fmt.Errorf("creating profile for over_scheduler name %s: %v", cfg.SchedulerName, err)
		}
...
// Map holds frameworks indexed by over_scheduler name.
type Map map[string]framework.Framework

// HandlesSchedulerName returns whether a profile handles the given over_scheduler name.
