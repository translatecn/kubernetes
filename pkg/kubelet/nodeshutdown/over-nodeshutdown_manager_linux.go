...
const (
	...
	nodeShutdownMessage            = "由于节点即将关闭,Pod被终止."
...
type dbusInhibiter interface {
	CurrentInhibitDelay() (time.Duration, error)              // 系统关闭或睡眠请求延迟的最长时间
	InhibitShutdown() (systemd.InhibitLock, error)            // 用于阻止系统关机
	ReleaseInhibitLock(lock systemd.InhibitLock) error        // 用于释放之前获取的关机锁
	ReloadLogindConf() error                                  // 用于重新加载 logind 配置
	MonitorShutdown() (<-chan bool, error)                    // 监视系统关机事件
	OverrideInhibitDelay(inhibitDelayMax time.Duration) error // 用于覆盖   系统关闭或睡眠请求延迟的最长时间
}
...
func (m *managerImpl) start() (chan struct{}, error) {
	...
	currentInhibitDelay, err := m.dbusCon.CurrentInhibitDelay() // 系统关闭或睡眠请求延迟的最长时间
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		for {
			...
			case isShuttingDown, ok := <-events:
				...
				if isShuttingDown {
					m.recorder.Event(m.nodeRef, v1.EventTypeNormal, kubeletevents.NodeShutdown, "关机管理器检测到关机事件")
				} else {
					m.recorder.Event(m.nodeRef, v1.EventTypeNormal, kubeletevents.NodeShutdown, "关机管理器检测到关机取消")
				}
