...
type KillPodOptions struct {
	CompletedCh chan<- struct{} // 终止完成的回调,close 它
	...
	PodTerminationGracePeriodSecondsOverride *int64        // 优雅关闭的时间
}
...
// PodWorkerState 用于分类表示 Pod 的状态,以供 PodWorker 使用.包含以下四个枚举值：
//
// 1. setup（同步）：表示正在设置 Pod.
// 2. teardown of containers（容器正在停止）：表示正在停止 Pod 中的容器.
// 3. cleanup（清理）：表示正在清理 Pod.
// 4. recreated with the same UID（使用相同 UID 重新创建）：表示在停止 Pod 的同时收到重新创建 Pod 的请求,需要使用相同的 UID 来重新创建.
type PodWorkerState int

const (
	SyncPod                   PodWorkerState = iota // 表示 Pod 应该已经启动并正在运行.
	TerminatingPod                                  // 表示 Pod 不再设置,但某些容器可能正在运行并被停止.
	TerminatedPod                                   // 表示 Pod 已停止,不能再有运行的容器,可以执行任何前台清理.
	TerminatedAndRecreatedPod                       // 表示在 Pod 被终止后收到重新创建 Pod 的请求.Pod 被终止并可以通过向 PodWorker 发送创建事件来重新启动.
)

type podWork struct {
	WorkType PodWorkType      // 表示要执行的同步类型,可以是 sync（创建）、terminating（停止容器）或 terminated（清理并写入状态）.
	Options  UpdatePodOptions // 同步 Pod 状态时传递数据.
}
...
type podSyncStatus struct {
	...
	working               bool              // 如果pod worker当前处于sync方法中,working为true.
	fullname              string            //
	syncedAt              time.Time         // pod worker 第一次观测到这个pod的时间
	terminatingAt         time.Time         // 开始设置为终止的时间,只设置一次,异步终止
	terminatedAt          time.Time         // 在pod worker成功完成 syncTerminatingPod 调用后设置,这意味着所有正在运行的容器都将停止.
	startedTerminating    bool              // 指示 pod worker 是否已经观察到停止 pod 的请求.
	deleted               bool              // 如果pod已在 api-sserver 上标记为删除,或者没有表示任何配置(之前已删除),则为true.
	...
	notifyPostTerminating []chan<- struct{} // 将在 pod 转换为 terminated 状态后被关闭.在 pod 处于 terminated 状态后,不应再向此列表中添加任何内容.
...
type podWorkers struct {
	...
	startedStaticPodsByFullname        map[string]types.UID   // 通过全名跟踪已经启动的静态pod的所有id
	waitingToStartStaticPodsByFullname map[string][]types.UID // 通过全名跟踪正在等待启动的静态pod的所有id
...
// SyncKnownPods 清除已完全终止且不在 desiredPods 列表中的 Pod.
...
// 这段代码定义了一个 `allowStaticPodStart` 函数,用于尝试启动静态 Pod,并返回 true,如果：
// 1. 没有其他已经启动的具有相同全名的静态 Pod
// 2. UID 与第一个等待启动的有效静态 Pod 相匹配.
//
// 确保静态 Pod 不会重复启动,并且只有具有相同 UID 的第一个等待启动的有效静态 Pod 才能启动.
// 这是为了确保每个静态 Pod 都有唯一的标识符,并且不会在同一时间内重复运行.如果 `allowStaticPodStart` 返回 false,则表示静态 Pod 已经在运行或有其他问题,需要进行进一步的排查.
...
// 用于返回或初始化已知 worker 的适当上下文.如果当前上下文已过期,则会重置上下文.如果没有 worker 存在,则不返回任何上下文.该函数的作用是为已知的 worker 获取上下文,并确保上下文是最新的.如果 worker 不存在,则不返回上下文.
...
func (p *podWorkers) completeWork(pod *v1.Pod, phaseTransition bool, syncErr error) {
	...
	switch {
	case phaseTransition: // Kubernetes 控制器会相应地更新 Pod 的状态,并通知相应的组件.
...
// completeTerminatingRuntimePod 函数是在 syncTerminatingPod 成功完成时调用的,这意味着一个没有配置的孤立的pod被终止了,我们可以退出循环并进行清理.
// 由于孤立的pod没有API表示,因此我们希望在这一点上退出循环并进行清理.这将更新终止状态,防止未来的同步,并确保其他kubelet循环知道此pod不在运行任何容器.
...
func (p *podWorkers) UpdatePod(options UpdatePodOptions) { // ✅
	...
	if options.RunningPod != nil { // 正在运行的 pod
		if options.Pod == nil { // 全部pod
	...
	// 决定怎么处理这个 pod  我们要么建好它,要么拆掉它,要么无视它
	now := time.Now()
	status, ok := p.podSyncStatuses[uid]
	if !ok {
	...
	case status.IsTerminated():
		...
		if isRuntimePod {
			klog.V(3).InfoS("Pod正在等待终止,忽略仅运行时的kill,直到Pod worker完全终止", "pod", klog.KObj(pod), "podUID", pod.UID)
			return
		}
	...
	case status.IsTerminationRequested():
		...
		gracePeriod, gracePeriodShortened := calculateEffectiveGracePeriod(status, pod, options.KillPodOptions) // 重新计算优雅关闭的时间
	...
	if !exists {
		...
		if p.workerChannelFn != nil {
			outCh = p.workerChannelFn(uid, podUpdates) // 钩子函数
		} else {
			outCh = podUpdates
		}
...
// allowPodStart 是否允许pod启动   (可以开始,永远都可以开始)
...
func (p *podWorkers) managePodLoop(podUpdates <-chan podWork) { // ✅
	...
	for update := range podUpdates {
		...
		err := func() error {
			...
			//	 当我们收到一个运行中的pod,我们根本不需要status
			default:
				// 具体来说,当前代码在转换 Pod 的状态时需要等待 PLEG 通过缓存发送下一个刷新的信号,最长等待时间为 2 秒.
				// 这会增加大约 1 秒的延迟,影响状态从同步到终止的转换,以及所有终止重试（包括清除操作）.
				// 为了改善这种延迟,可以让 PLEG 持续运行,并在关键事件发生时（如 killPod、sync->terminating）刷新 Pod 状态变化.
				// 改善这种延迟也可以减少终止容器状态被垃圾回收之前我们更新 API 服务器的机会（从而丢失退出码）的可能性.
				status, err = p.podCache.GetNewerThan(pod.UID, lastSyncTime) // 获取pod 最新的状态
			}
			if err != nil {
		...
		case update.WorkType == TerminatingPodWork:
			...
			// 如果一个 pod 在配置中不存在了,它就不需要被手动终止,因为 Kubernetes 会自动进行垃圾回收,将这些无用的 pod 清理掉.
		...
		case isTerminal:
			// 如果 syncPod 表示现在我们处于终止状态,那么就将相应的 Pod 状态设置为 Terminating,以便将该 Pod 从 Kubernetes 中删除.
			klog.V(4).InfoS("Pod is terminal", "pod", klog.KObj(pod), "podUID", pod.UID, "updateType", update.WorkType)
			p.completeSync(pod)
			phaseTransition = true
		}

		// 如果有必要,将重试排队,然后将下一个事件放在通道中(如果有)
