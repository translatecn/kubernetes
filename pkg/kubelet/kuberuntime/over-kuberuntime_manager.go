...
type podActions struct {
	KillPod                    bool                                              // 是否需要停止 Pod 中所有正在运行的容器（包括普通容器、init 容器和临时容器）以及沙箱.
	CreateSandbox              bool                                              // 是否需要创建一个新的沙箱.如果需要杀死 Pod 并创建一个新的沙箱,则需要清除所有 init 容器（即删除它们）.
	SandboxID                  string                                            // 现有沙箱的 ID.它用于在 ContainersToStart 中启动容器.
	Attempt                    uint32                                            // 创建 Pod 沙箱的尝试次数.
	NextInitContainerToStart   *v1.Container                                     // 下一个要启动的 init 容器.
	ContainersToStart          []int                                             // 包含要启动的容器索引列表,其中索引是 Pod 规范中特定容器的索引.
	ContainersToKill           map[kubecontainer.ContainerID]containerToKillInfo // 包含需要终止的容器的映射.注意,键是容器的容器 ID,而值包含终止容器所需的必要信息.
	EphemeralContainersToStart []int                                             // 包含要启动的临时容器索引列表,其中索引是 Pod 规范中特定容器的索引.
}
...
func (m *kubeGenericRuntimeManager) computePodActions(pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	if createPodSandbox {
		...
		var containersToStart []int // 需要继续启动的容器
...
// SyncPod 通过执行以下步骤将正在运行的pod同步到所需的pod:
//
// 1.计算沙箱和容器更改.
// 2.必要时杀死pod沙箱.
// 3.杀死任何不应该运行的容器.
