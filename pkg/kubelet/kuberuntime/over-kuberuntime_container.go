...
func (m *kubeGenericRuntimeManager) generateContainerConfig(ctx context.Context,
	...
	containerLogsPath := buildContainerLogsPath(container.Name, restartCount) // kube-apiserver/0.log
...
// makeMounts 为kubelet运行时v1生成容器卷挂载.
...
// findNextInitContainerToRun 返回上一个失败容器的状态,下一个启动的初始化容器的索引,如果没有进一步的初始化容器,则返回done.
// 只有当init容器失败时才返回Status,在这种情况下next将指向当前容器.
...
// 移除所有init容器.注意,这个函数不检查容器的状态,因为它假设在调用发生之前所有init容器都已停止.
...
// startContainer启动一个容器,并返回一个消息,指出错误时失败的原因.
// 通过以下步骤启动容器:
// *创建容器
// *启动容器
// *运行post start生命周期钩子(如果适用)
func (m *kubeGenericRuntimeManager) startContainer(ctx context.Context, podSandboxID string, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, podIP string, podIPs []string) (string, error) {
	...
	if containerStatus != nil {
		...
		// /var/log/pods/aps-os_operationcenter-6fb5fc485-d94cg_c96667f1-5d8a-4dd7-bf7e-d72b15d111ee/app-container/0.log
		// /var/log/pods/aps-os_operationcenter-6fb5fc485-d94cg_c96667f1-5d8a-4dd7-bf7e-d72b15d111ee/app-container/1.log
		logDir := BuildContainerLogsDirectory(pod.Namespace, pod.Name, pod.UID, container.Name)
		restartCount, err = calcRestartCountByLogDir(logDir) // 从日志文件获取重启次数
		if err != nil {
	...
	// Symlink容器日志到旧的容器日志位置,以支持集群日志记录.
	// TODO(random-liu): Remove this after cluster logging supports CRI container log path.
...
// 尝试从path表示的CRI日志的末尾读取最大日志长度.它读取最多的日志行.
...
// 在文件系统中查找所提供的终止消息路径,返回这些字节的有限数量,或者在应该检查日志时返回true.
