...
var _ = SIGDescribe("ResourceQuota", func() {
	...
	*/
	framework.ConformanceIt("should create a ResourceQuota and capture the life of a pod.", func() {
		...
		pod = newTestPodForQuota(f, "fail-pod", requests, v1.ResourceMap{})
	...
	*/
	framework.ConformanceIt("should verify ResourceQuota with best effort scope.", func() {
		...
		pod := newTestPodForQuota(f, podName, v1.ResourceMap{}, v1.ResourceMap{})
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
	...
	framework.ConformanceIt("should apply changes to a resourcequota status", func() {
		...
		w := &cache.ListWatch{
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				...
				return rqClient.Watch(context.TODO(), options)
			},
		}

		rqList, err := f.ClientSet.CoreV1().ResourceQuotas("").List(context.TODO(), metav1.ListOptions{LabelSelector: labelSelector})
		...
		initialResourceQuota, err := rqClient.Get(context.TODO(), rqName, metav1.GetOptions{})
		framework.ExpectNoError(err)
		...
		hardLimits := quota.Add(v1.ResourceMap{}, initialResourceQuota.Spec.Hard)
		...
		hardLimits = quota.Add(v1.ResourceMap{}, xResourceQuota.Spec.Hard)
		...
		repatchedResourceQuota, err := rqClient.Patch(context.TODO(), rqName, types.StrategicMergePatchType,
			[]byte(`{"status": {"hard": `+string(rqStatusJSON)+`}}`),
			metav1.PatchOptions{}, "status")
		framework.ExpectNoError(err)
		...
		err = wait.PollImmediate(5*time.Second, 5*time.Minute, func() (bool, error) {
			resourceQuotaResult, err := rqClient.Get(context.TODO(), rqName, metav1.GetOptions{})
			framework.ExpectNoError(err)
...
var _ = SIGDescribe("ResourceQuota [Feature:PodPriority]", func() {
	...
	ginkgo.It("should verify ResourceQuota's priority class scope (quota set to pod count: 1) against a pod with same priority class.", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass1")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
	...
	ginkgo.It("should verify ResourceQuota's priority class scope (quota set to pod count: 1) against 2 pods with same priority class.", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass2")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		...
		pod2 := newTestPodForQuotaWithPriority(f, podName2, v1.ResourceMap{}, v1.ResourceMap{}, "pclass2")
	...
	ginkgo.It("should verify ResourceQuota's priority class scope (quota set to pod count: 1) against 2 pods with different priority class.", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass3")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		...
		podName2 := "testpod-pclass2-2"
		pod2 := newTestPodForQuotaWithPriority(f, podName2, v1.ResourceMap{}, v1.ResourceMap{}, "pclass3")
	...
	ginkgo.It("should verify ResourceQuota's multiple priority class scope (quota set to pod count: 2) against 2 pods with same priority classes.", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass5")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		...
		pod2 := newTestPodForQuotaWithPriority(f, podName2, v1.ResourceMap{}, v1.ResourceMap{}, "pclass6")
		pod2, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod2, metav1.CreateOptions{})
		framework.ExpectNoError(err)
	...
	ginkgo.It("should verify ResourceQuota's priority class scope (quota set to pod count: 1) against a pod with different priority class (ScopeSelectorOpNotIn).", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass7")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
	...
	ginkgo.It("should verify ResourceQuota's priority class scope (quota set to pod count: 1) against a pod with different priority class (ScopeSelectorOpExists).", func() {
		...
		pod := newTestPodForQuotaWithPriority(f, podName, v1.ResourceMap{}, v1.ResourceMap{}, "pclass8")
		pod, err = f.ClientSet.CoreV1().Pods(f.Namespace.Name).Create(context.TODO(), pod, metav1.CreateOptions{})
		framework.ExpectNoError(err)
...
var _ = SIGDescribe("ResourceQuota", func() {
	...
	ginkgo.It("should verify ResourceQuota with cross namespace pod affinity scope using scope-selectors.", func() {
		...
		ginkgo.By("Ensuring ResourceQuota status is calculated")
		wantUsedResources := v1.ResourceMap{v1.ResourcePods: resource.MustParse("0")}
...
func newTestResourceQuotaWithScopeForPriorityClass(name string, hard v1.ResourceMap, op v1.ScopeSelectorOperator, values []string) *v1.ResourceQuota {
	return &v1.ResourceQuota{
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec: v1.ResourceQuotaSpec{Hard: hard,
			ScopeSelector: &v1.ScopeSelector{
...
func newTestPodForQuota(f *framework.Framework, name string, requests v1.ResourceMap, limits v1.ResourceMap) *v1.Pod {
	return &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
...
func newTestPodForQuotaWithPriority(f *framework.Framework, name string, requests v1.ResourceMap, limits v1.ResourceMap, pclass string) *v1.Pod {
	return &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
...
func waitForResourceQuota(c clientset.Interface, ns, quotaName string, used v1.ResourceMap) error {
