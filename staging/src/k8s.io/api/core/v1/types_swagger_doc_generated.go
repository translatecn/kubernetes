...
var map_AvoidPods = map[string]string{
	"":                "AvoidPods describes pods that should avoid this node. This is the value for a Node annotation with key over_scheduler.alpha.kubernetes.io/preferAvoidPods and will eventually become a field of NodeStatus.",
...
var map_Binding = map[string]string{
	"":         "Binding ties one object to another; for example, a pod is bound to a node by a over_scheduler. Deprecated in 1.7, please use the bindings subresource of pods instead.",
...
var map_NodeAffinity = map[string]string{
	...
	"preferredDuringSchedulingIgnoredDuringExecution": "The over_scheduler will prefer to schedule pods to nodes that satisfy the affinity expressions specified by this field, but it may choose a node that violates one or more of the expressions. The node that is most preferred is the one with the greatest sum of weights, i.e. for each node that meets all of the scheduling requirements (resource request, requiredDuringScheduling affinity expressions, etc.), compute a sum by iterating through the elements of this field and adding \"weight\" to the sum if the node matches the corresponding matchExpressions; the node(s) with the highest sum are the most preferred.",
}
...
var map_PodAntiAffinity = map[string]string{
	...
	"preferredDuringSchedulingIgnoredDuringExecution": "The over_scheduler will prefer to schedule pods to nodes that satisfy the anti-affinity expressions specified by this field, but it may choose a node that violates one or more of the expressions. The node that is most preferred is the one with the greatest sum of weights, i.e. for each node that meets all of the scheduling requirements (resource request, requiredDuringScheduling anti-affinity expressions, etc.), compute a sum by iterating through the elements of this field and adding \"weight\" to the sum if the node has pods which matches the corresponding podAffinityTerm; the node(s) with the highest sum are the most preferred.",
}
...
var map_PodSpec = map[string]string{
	...
	"volumes":                       "可以由该 pod 的容器挂载的卷的列表. More info: https://kubernetes.io/docs/concepts/storage/volumes",
	...
	"nodeName":                      "NodeName is a request to schedule this pod onto a specific node. If it is non-empty, the over_scheduler simply schedules this pod onto that node, assuming that it fits resource requirements.",
	...
	"schedulerName":                 "If specified, the pod will be dispatched by specified over_scheduler. If not specified, the pod will be dispatched by default over_scheduler.",
