...
type Scheme struct {
	...
	typeToGVK map[reflect.Type][]schema.GroupVersionKind // 已知的所有类型，包括pod option 等等
	...
	// 组到这些组的有序版本列表的映射是否表明这些版本在方案中注册的默认优先级
	versionPriority map[string][]string // group:[v1,v2]
...
func (s *Scheme) Convert(in, out interface{}, context interface{}) error {
	...
	case okOut:
		...
		if unversioned || gvk.Version != APIVersionInternal { // 内部版本、无版本
...
//
// VersionsForGroupKind 返回给定组中特定GroupKind可以转换为的版本。
// GroupKind可以转换为不同的组。该信息可在equivalresourcemapper中获得。
...
func (s *Scheme) PrioritizedVersionsForGroup(group string) []schema.GroupVersion { // 优先
	ret := []schema.GroupVersion{}
	// 观测到、且在versionPriority 里的group
	// 返回给定组中特定GroupKind可以转换为的版本
...
var internalPackages = []string{"k8s.io/apimachinery/pkg/runtime/over_scheme.go"}
